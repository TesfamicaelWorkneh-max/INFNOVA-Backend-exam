...
export class CoursesService {
    ...
    private courseId:number = 1; //auto-increment id
       ...
       findOne(id:number):Course{//Return type is Course; ensures the method returns Course object matching the interface
       ...
       create(dto:CreateCourseDto){ /* this part is just like req.body comes from the route will be
  ...
  Object.assign(course, dto); // only updates fields present in dto
  return course;
}
  remove(id: number): { message: string } { //will return object with value string
  const course = this.courses.find(c => c.id === id);
  if (!course) throw new NotFoundException(`Course with ID ${id} not found`);//404

  this.courses = this.courses.filter(c => c.id !== id); // remove by filtering
